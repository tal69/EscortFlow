# -------------------------------------------------------------------------------
# Name:        EscortFlowSimRT  - NOT A GOOD IDEA
...
#              Realtime version meaning that the computation time affects actual timing
#              If the computation is delayed beyond execution horizon time - the syste waits idle
#              If computation is short, next cycle computation may start earlier
#
...
#              28/12/2023 Real time version that adjust to the solution time
#
...
parser.add_argument("-T", "--fractional_horizon", type=int,
                    help="Number of fractional periods in model (default 20)",
                    default=20)
...
parser.add_argument("-k", "--takt_time", type=float, help="How many seconds each takt takes (default 1)", default=1)

parser.add_argument("-a", "--export_animation", action="store_true",
                    help="Export animation files, one for each instance")
parser.add_argument("-b", "--bm", action="store_true",
                    help="Simultaneous block movement regime (otherwise SLM, aka LM, NBM)")
...
block_arrival = np.zeros(num_of_blocks, dtype=int)
block_total_lead_time = np.zeros(num_of_blocks, dtype=int)
...
if args.header_line:
    f.write(
        f"\ndate, SBM/SLM, Retrieval Mode, Queue Management, Lx x Ly, #IOs, # Escorts, IOs, Request rate, gamma, "
        f"distance_penalty, time_penalty, seed , T fractional, T integer, T execution, cpu time_limit, max balls in air, "
        f"T sim, Warmup loads, T actual, Total lead time, # loads entered, mean lead time,  95% C.I, #load movements, total CPU time, "
        f"Sim iterations, Idle takts, Non optimal iter, max gap, Heuristic solution")
    if args.print_blocks:
        for i in range(num_of_blocks):
            f.write(f", B{i} - lead time")
        for i in range(num_of_blocks):
...
while True:
    ...
    if load_queue and len(A) < args.max_balls_in_air:

        if args.queue_management == 'spt':
            # rank the loads in the queue by their distance from O / number of requests
            load_queue = sorted(load_queue, key= lambda q: dist[load_loc[q][0], load_loc[q][1]] / len(requests_on_load[q]) )

        to_remove = min(len(load_queue), args.max_balls_in_air - len(A))
        for i in range(len(load_queue) - to_remove, len(load_queue)):
            load_to_remove = load_queue.pop(0)
            new_target_loads_xy.add(load_loc[load_to_remove])  # just for the animation
            A.append(load_loc[load_to_remove])
            if args.log:
                f = open(sim_log_file, "a")
                f.write(
                    f"Time: {curr_t}: load #{load_to_remove} with requests {requests_on_load[load_to_remove]} currently @ {load_loc[load_to_remove]} is removed from the queue\n")
                f.close()

    """  Draw request form request list until current time"""
    while req_count < number_of_requests:
        if arrivals[req_count] < curr_t:
            ...
            if args.log:
                f = open(sim_log_file, "a")
                f.write(
                    f"Time: {arrivals[req_count]}: request #{req_count} arrive, load:{req2load[req_count]}, currently @ {load_loc[req2load[req_count]]}\n")
                f.close()
    ...
    if args.log:
        f = open(sim_log_file, "a")
        f.write(f"      iteration {sim_iter}, BEFORE -- solution time: {curr_t - args.exec_horizon}-{curr_t} \n")
    ...
    try:
        if args.bm:
            ...
            subprocess.run(["oplrun", "escort_flow_lm_rh.mod", dat_file], check=True)
    except:
        ...
        if cplex_status not in [1, 11, 102] or (
            ...
            for i in range(args.exec_horizon):
                ...
                new_moves.append(one_step_move)
            ...
            makespan_rh, flowtime_rh, NumberOfMovements_rh = int(s[4]), int(s[5]), int(s[6])
            obj_rh, lb_rh = float(s[7]), float(s[8])

            if cpu_time_iter > time_limit:
                non_optimal += 1
            NumberOfMovements += NumberOfMovements_rh
            max_gap = max(max_gap, (obj_rh - lb_rh) / max(obj_rh, 1))
            if args.log:
                f = open(sim_log_file, "a")
                f.write(
                    f"      iteration {sim_iter}, status:{cplex_status} LB={lb_rh}, ob={obj_rh} "
            ...
            mvs = f.readlines()
            f.close()
            new_moves = eval(mvs[-1])

        for t in range(len(new_moves)):
            for (loc1, loc2) in new_moves[t]:
            ...
            for loc in O:
                if pbs[loc[0], loc[1]] != 0:
                    ...
                    for req in requests_on_load[leaving_load]:
                        total_lead_time += ((curr_t + t) - arrivals[req])
                        departures[req] = curr_t + t
                        block_total_lead_time[arrivals[req] // args.block_length] += ((curr_t + t) - arrivals[req])
                        block_arrival[arrivals[req] // args.block_length] += 1
                        if args.log:
                            f = open(sim_log_file, "a")
                            f.write(
                                f"Time: {curr_t + t} request #{req} departs via ({loc[0]}, {loc[1]}) with load {leaving_load}\n")
                            # f.write(f"      E = {E}, A = {A}, Q = {load_queue}\n")
                            f.close()
                    ...
                    if leaving_load in load_queue:
                        if args.log:
                            f = open(sim_log_file, "a")
                            f.write(
                                f"Time: {curr_t + t} load {leaving_load} accidentally arrived at an output cell and was removed from the queue\n")
                            f.close()
                        load_queue.pop(load_queue.index(leaving_load))

        """ Advance clock if optimization was too long """
        extra_takts = max(0, int(0.8+cpu_time_iter/args.takt_time) - args.exec_horizon)
        if extra_takts:
            for i in range(extra_takts):
                moves.append([])  # idle cycle
            if args.log:
                f = open(sim_log_file, "a")
                f.write(f"Time: {curr_t} add {extra_takts}  computation takts\n")
                f.close()
            curr_t += extra_takts
            computation_takt += extra_takts
        ...
        for l, loc in load_loc.items():
            if requests_on_load[l]:  # there are still requests on the load
                if loc not in A and l not in load_queue:
                    print(
                        f"************ Panic: load {l} at location {loc} with requests {requests_on_load[l]} got lost")
                    if args.log:
                        f = open(sim_log_file, "a")
                        f.write(
                            f"************ Panic: load  Panic: load {l} at location {loc} with requests {requests_on_load[l]} got lost\n")
                        f.close()

        for l in new_target_loads_xy:
            new_moves[0].insert(0, (l, l))
        moves += new_moves  # read moves in the current horizon
...
if args.export_animation:
    pickle.dump((Lx, Ly, O, E_orig, A_orig, moves),
                ...
                open(
                    f"script_sim_{'SBM' if args.bm else 'SLM'}_RT_{Lx}_{Ly}_{args.escorts_num}_{args.request_rate}_{args.simulation_length}.p",
                    "wb"))
...
f = open(result_csv_file, 'a')
f.write(
    f"{time.ctime()},{'SBM' if args.bm else 'SLM'}, continue, {args.queue_management},{Lx}x{Ly}, {len(O)}, {len(E_orig)}, "
    f"{tuple_opl(O)},{args.request_rate}, {gamma}, {args.distance_penalty}, {args.time_penalty},{args.seed},"
    f" {args.fractional_horizon}, {args.integer_horizon}, {args.exec_horizon},{time_limit}, {args.max_balls_in_air}, "
    f"{args.simulation_length}, {args.warmup_arrivals}, {curr_t}, {total_lead_time},{number_of_requests}, "
    f"{lead_time_mean:.3f}, {half_ci:.3f}, {NumberOfMovements},{cpu_time:.2f}, {sim_iter}, {idle_takt}, {non_optimal}, {max_gap:.4f}, {heuristic_sol}")

if args.print_blocks:
    for i in range(num_of_blocks):
        f.write(f",{(block_total_lead_time[i]/block_arrival[i]):.3f}")
    for i in range(num_of_blocks):
