...
parser.add_argument("-M", "--max_attention", dest="max_attention", type=int,
                    help="Maximum attention limit, i.e. the number of target loads considered concurrently; if omitted, all cells are eligible",
                    default=None)
parser.add_argument("--max_balls_in_air", dest="max_attention", type=int, help=argparse.SUPPRESS)
parser.add_argument('-q', '--queue_management', choices=['fifo', 'spt'],
                    help='Select queue management strategy when there are too many open requests to handle (more than '
                         'the attention limit): fifo or spt (default) - closest load with most requests'
                         , default='spt')
...
Lx = args.Lx
Ly = args.Ly
max_attention_csv = "n/a" if args.max_attention is None else args.max_attention
if args.max_attention is None:
    args.max_attention = Lx * Ly
...
header_cols = [
    ...
    "Fractional Horizon", "Integer Horizon", "Epoch", "Time Limit", "Attention Limit",
    "Actual Attention", "Max actual opt gap", "Max allowed opt gap",
    ...
    while True:
        ...
        if not open_requests:
            ...
            if args.greedy:
                ...
                target_loads = eligible_target_loads[:args.max_attention]
