...
#              EVEN IF THEIR RELATED LOADS ARE MOVING. BETTER USE V2
...
parser.add_argument("-u", "--heuristic_only", action="store_true",
                    help="Run only heuristic (get stuck when the number of output cells is not very large)")
...
while True:
    ...
    while req_count < number_of_requests:
        if arrivals[req_count] < curr_t:
            ...
            if load_loc[req2load[req_count]] in O: # we are lucky ad the request if for a load that s currently at a depot
                if args.log:
                    f = open(sim_log_file, "a")
                    f.write(
                        f"Time: {curr_t}: request #{req_count} load:{req2load[req_count]}, currently @ {load_loc[req2load[req_count]]} which is an output cell. our lucky day\n")
                    f.close()
                    departures[req_count] = curr_t

                    total_lead_time += (curr_t - arrivals[req_count])
                    block_total_lead_time[arrivals[req_count] // args.block_length] += (curr_t  - arrivals[req_count])
                    block_arrival[arrivals[req_count] // args.block_length] += 1
            elif load_loc[req2load[req_count]] not in A:
    ...
    try:
        ...
        if args.heuristic_only or cplex_status not in [1, 11, 102] or ilp_gap > args.max_opt_gap or (
...
if args.export_animation:
    pickle.dump((Lx, Ly, O, E_orig, A_orig, moves),
                ...
                open(
                    f"script_sim_{'SBM' if args.bm else 'SLM'}_{'HUE' if args.heuristic_only else 'ILP'}_{Lx}_{Ly}_{args.escorts_num}_{args.request_rate}_{args.simulation_length}.p",
                    "wb"))
