...
#
# Created:     22-8-2020
# Updated      5-12-2023 facilitate 'continue' in multi-load retrieval problems
...
class PathAnimation:
    def __init__(self, box_size=60, offset=40):
        self.Lx = L[0]  # dimensions of the warehouses
        self.Ly = L[1]
        ...
        # Esav is probably redundant now because we don't update self.E anymore

        self.box_size = box_size  # size of the square representation of each load
        self.offset = offset
        ...
        self.time_step = 0.15  # change this to control animation speed

        self.master = Tk()
        self.master.title("PBS-Flow animation by Tal Raviv (2023)")
        ...
        self.cnvs = Canvas(self.master, width=box_size * L[0] + 2 * offset, height=box_size * L[1] + 2 * offset,
                           bg="white")
        self.cnvs.pack()
        ...
        for t in O:
            x, y = t
            self.cnvs.create_rectangle(self.offset + x * self.box_size, self.offset + (self.Ly - y - 1) * self.box_size,
                                       self.offset + (x + 1) * self.box_size,
                                       self.offset + (self.Ly - y) * self.box_size, width=0, fill="green")
            ...
            for y in range(self.Ly):
                if not (x, y) in self.E:
                    if not (x, y) in self.R:
                        self.LoadsRef[x][y] = self.cnvs.create_rectangle(self.offset + 4 + x * self.box_size,
                                                                         self.offset + 4 + (
                                                                                     self.Ly - y - 1) * self.box_size,
                                                                         self.offset - 5 + (x + 1) * self.box_size,
                                                                         self.offset - 5 + (
                                                                                     self.Ly - y) * self.box_size,
                                                                         fill="blue")
                    else:
                        self.LoadsRef[x][y] = self.cnvs.create_rectangle(self.offset + 4 + x * self.box_size,
                                                                         self.offset + 4 + (
                                                                         ...
                                                                         self.offset - 5 + (x + 1) * self.box_size,
                                                                         self.offset - 5 + (
                                                                         (self.Ly - y)) * self.box_size, fill="red")
    ...
    def one_step(self, cont=False):
        ...
        # Change the color of load that arrive at the output cell to blue (will affect only target loads)
        for i in range(len(moves[self.t])):
            mv = moves[self.t][i]
            #  New version 24/12/23  - target loads turn blue when they enter the output cell (better for 'continue')
            if mv[1] in O and mv[1] != mv[0]:
                self.cnvs.itemconfig(Load_handle[i], fill='blue')
...
if __name__ == '__main__':
    ...
    # cut the py extension for the import operation
